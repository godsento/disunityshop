...
void Slider::draw( ) {
	...
	if (g_menu.main.config.gradientslider.get()) {
		render::gradient1337(p.x + SLIDER_X_OFFSET + 1, p.y + m_offset + 1, m_fill - 2, SLIDER_HEIGHT - 2, color,slider);
		render::rect_filled_fade(p.x + SLIDER_X_OFFSET + 1, p.y + m_offset + 1, m_fill - 2, SLIDER_HEIGHT - 2, { 50, 50, 35, m_parent->m_alpha }, 0, 150);
	}
	else if (!g_menu.main.config.gradientslider.get()) {
