...
void Visuals::Hitmarker() {
	...
	if (m_hit_duration <= 0.f)
		return;
	int curtime = g_csgo.m_globals->m_curtime;
	...
	for (int hm = 6; hm < 12; hm++)
	{
			render::rect_filled(x / 2 + hm, y / 2 + hm, 1, 1, { 255,255,255,alpha });
			render::rect_filled(x / 2 - hm, y / 2 - hm, 1, 1, { 255,255,255,alpha });
			render::rect_filled(x / 2 - hm, y / 2 + hm, 1, 1, { 255,255,255,alpha });
			render::rect_filled(x / 2 + hm, y / 2 - hm, 1, 1, { 255,255,255,alpha });	
	}

	if (!g_menu.main.misc.hitmarkerop.get(0))
		return;
	...
	for (int i{ 1 }; i <= g_csgo.m_globals->m_max_clients; ++i) {
		...
		{
			render::esp.string(x / 2, y / 2 + 10, { 255,0,0,alpha }, tfm::format(XOR("edshort >_< %i"), out), render::ALIGN_CENTER);
		} if (!g_shots.iHeadshot)
		{
			render::esp.string(x / 2, y / 2 + 10, { 255,255,255,alpha }, out, render::ALIGN_CENTER);
		}
...
void Visuals::Indicators()
{
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::indicator.string(g_cl.m_width / 2, (g_cl.m_height / 2 + 28) + (i * 15), colors::white, indicator.text, render::ALIGN_CENTER);
	}
}
...
void Visuals::Spectators() {
	...
	for (size_t i{ }; i < spectators.size(); ++i) {
		...
		render::esp.string(g_cl.m_width - 15, (g_cl.m_height + 15) + (i * 15), colors::white, spectator, render::ALIGN_CENTER);
	}
}
...
void Visuals::think() {
	...
	if (g_menu.main.visuals.localdebug.get()) 
	{
		render::esp.string(4, g_cl.m_height / 2, colors::white, tfm::format("ANGX> %S | ANGY> %S |", g_cl.m_angle.x, g_cl.m_angle.y));
		render::esp.string(4, g_cl.m_height / 2 + 10, colors::white, tfm::format("RANGX> %S | RANGY> %S | ", g_cl.m_radar.x, g_cl.m_radar.y));
		render::esp.string(4, g_cl.m_height / 2 + 20, colors::white, tfm::format("SEQ> %S | WEIGHT> %S | CY> %S ", g_cl.m_local->m_AnimOverlay()[3].m_sequence, g_cl.m_local->m_AnimOverlay()[3].m_weight, g_cl.m_local->m_AnimOverlay()[3].m_cycle));
	}
	...
	if (!g_cl.m_processing) {
		...
		for (size_t i{ impact_count }; i-- > 0; ) {
			...
			if (is_final_impact) {
				...
				beam_info.m_flAmplitude = 2.f;   // beam 'jitter'.
