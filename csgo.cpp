...
bool CSGO::init( ) {
	...
	cl_clock_correction = m_cvar->FindVar(HASH("cl_clock_correction"));
	r_aspectratio = m_cvar->FindVar(HASH("r_aspectratio"));
	sv_occlude_players = m_cvar->FindVar(HASH("sv_occlude_players"));
	...
	// clock correction
	if (cl_clock_correction->GetInt() != 0)
	{
		cl_clock_correction->m_flags &= ~(1 << 14); // FCVAR_CHEAT
		cl_clock_correction->SetValue(0);
	}
