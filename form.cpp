...
void Form::draw( ) {
	...
	// border.
	render::rect( m_x, m_y, m_width, m_height, { 30,30,30,m_alpha });
	render::rect( m_x + 1, m_y + 1, m_width - 2, m_height - 2, { 20,20,20,m_alpha });
	...
	render::rect( m_x + 4, m_y + 4, m_width - 8, m_height - 8, { 20,20,20,m_alpha });
	render::rect( m_x + 5, m_y + 5, m_width - 10, m_height - 10, { 30,30,30,m_alpha });
	...
	if( !m_tabs.empty( ) ) {
		...
		render::rect_filled( tabs_area.x, tabs_area.y, tabs_area.w, tabs_area.h, { 0, 0, 0, m_alpha } );
		...
		if( !m_active_tab->m_elements.empty( ) ) {
			...
			render::rect_filled( el.x, el.y, el.w, el.h, { 0,0,0,m_alpha });
